...
public class ModuleGui extends Gui<UHC> {
    ...
    public ModuleGui(UHC uhc) {
        super(uhc, (int) Math.ceil((ModuleRegistry.INSTANCE.availableModules().size() + 1) / 9D),
            "Module Settings");
    }
    ...
    public void build() {
        ...
        for (UHCModule module : uhcModules) {
            ItemFactory factory = new ItemFactory(module.getGuiItem());
            if (module.isLoaded()) {
                factory.glowing();
            }
            ...
            factory
                .lore(module.isLoaded() ? ChatColor.GREEN + "LOADED" : ChatColor.RED + "UNLOADED");
