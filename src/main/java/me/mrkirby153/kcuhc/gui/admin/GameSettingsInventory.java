...
import me.mrkirby153.kcuhc.module.endgame.EndgameGui;
...
public class GameSettingsInventory extends PropertyGui {
    ...
    public void build() {
        ...
        if (ModuleRegistry.isLoaded(EndgameScenarioModule.class)) {
            ItemFactory factory = new ItemFactory(Material.ENDER_PEARL).name("Endgame Scenario");
            if (EndgameScenarioModule.currentScenario() != null)
                factory.lore(ChatColor.WHITE + "Current: " + ChatColor.GOLD + EndgameScenarioModule.currentScenario().getName());
            addButton(41, factory.construct(), ((player, clickType) -> new EndgameGui(plugin, player)));
        }
